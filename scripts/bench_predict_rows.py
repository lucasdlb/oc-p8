#!/usr/bin/env python3
"""Benchmark — /predict/rows code path (Pydantic validation → InferencePipeline).

Measures startup phases, cold start, batch-size sweep, concurrency behavior,
memory growth, output determinism, and profiling for the row-oriented JSON path
(`/predict/rows`). All measurements are in-process (no HTTP/uvicorn overhead).

The full code path is measured: CSV row dicts → Pydantic ``ApplicationRow``
validation → ``PredictRequest`` construction → ``predict()`` → ``model.predict()``.

Usage::

    # Full baseline
    uv run python scripts/bench_predict_rows.py --name baseline

    # Smoke test (fast, validates behavior)
    uv run python scripts/bench_predict_rows.py --name smoke \\
...
from credit_risk_server.models.predictor import predict
...
logger = logging.getLogger("bench_predict_rows")
...
def parse_args() -> Config:
    p = argparse.ArgumentParser(
        description="Benchmark /predict/rows code path (Pydantic → InferencePipeline)",
...
def load_row_dicts(data_path: Path) -> list[dict]:
    """Load all application rows as dicts ready for Pydantic validation."""
...
def sample_rows(row_dicts: list[dict], batch_size: int, rng: random.Random) -> list[dict]:
    if batch_size >= len(row_dicts):
        return rng.choices(row_dicts, k=batch_size)
    return rng.sample(row_dicts, k=batch_size)
...
def measure_startup(cfg: Config) -> tuple[dict, InferencePipeline]:
    ...
    startup = {
        ...
        "total_ms": round(model_load_ms, 1),
    }
    sys.stderr.write(f"  model_load={startup['model_load_ms']:.0f}ms\n\n")
    sys.stderr.flush()
    return startup, model


def build_and_predict(model: InferencePipeline, rows: list[dict]) -> list[tuple[int, float]]:
    """Full /predict/rows code path: dict → ApplicationRow → PredictRequest → predict()."""
    app_rows = [ApplicationRow(**row) for row in rows]
...
def run_benchmark(cfg: Config) -> dict:
    ...
    # Phase 1: Startup
    startup, model = measure_startup(cfg)

    # Pre-sample row batches for reproducibility
    batches = {bs: sample_rows(row_dicts, bs, rng) for bs in cfg.batch_sizes}
    ...
    def cold_fn() -> object:
        return build_and_predict(model, cold_batch)
    ...
    def make_predict_fn(batch_size: int) -> Callable[[], object]:
        ...
        def _run() -> object:
            return build_and_predict(model, batch)
    ...
    if not cfg.skip_concurrency:
        ...
        def make_concurrent_fn(task_idx: int) -> Callable[[], object]:
            batch = sample_rows(row_dicts, 50, rng)
    ...
    if not cfg.skip_memory:
        ...
        def mem_fn() -> object:
            return build_and_predict(model, mem_batch)
    ...
    if not cfg.skip_determinism:
        ...
        def det_fn() -> list[float]:
            results = build_and_predict(model, det_batch)
            return [prob for _, prob in results]
    ...
    if not cfg.skip_profile:
        ...
        def profile_fn() -> object:
            return build_and_predict(model, profile_batch)

        profile_info = profile_predict(
            predict_fn=profile_fn,
            iterations=cfg.profile_iterations,
            output_path=cfg.out_dir / f"profile_predict_rows_{cfg.name}",
        )
...
def main() -> int:
    ...
    sys.stderr.write(
        f"Benchmark /predict/rows → name={cfg.name} | runs={cfg.runs} | "
    ...
    out_json = cfg.out_dir / f"predict_rows_{cfg.name}.json"
